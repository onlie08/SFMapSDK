...
/**
 SFMKVStorageItem is used by `SFMKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface SFMKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `SFMKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use SFMKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use SFMKVStorageTypeFile to get better performance.
 * You can use SFMKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, SFMKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    SFMKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    SFMKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    SFMKVStorageTypeMixed = 2,
};



/**
 SFMKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for SFMKVStorage is `initWithPath:type:`.
 ...
 */
@interface SFMKVStorage : NSObject
...
@property (nonatomic, readonly) SFMKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(SFMKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is SFMKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is SFMKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is SFMKVStorageTypeMixed, then the item.value will be saved to file
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(SFMKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 SFMKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is SFMKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is SFMKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is SFMKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable SFMKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable SFMKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `SFMKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<SFMKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `SFMKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<SFMKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
