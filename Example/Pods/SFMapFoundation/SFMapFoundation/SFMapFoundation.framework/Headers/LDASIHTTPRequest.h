...
@class LDASIDataDecompressor;

extern NSString *LDASIHTTPRequestVersion;
...
typedef enum _LDASIAuthenticationState {
	LDASINoAuthenticationNeededYet = 0,
	LDASIHTTPAuthenticationNeeded = 1,
	LDASIProxyAuthenticationNeeded = 2
} LDASIAuthenticationState;

typedef enum _LDASINetworkErrorType {
    LDASIConnectionFailureErrorType = 1,
    LDASIRequestTimedOutErrorType = 2,
    LDASIAuthenticationErrorType = 3,
    LDASIRequestCancelledErrorType = 4,
    LDASIUnableToCreateRequestErrorType = 5,
    LDASIInternalErrorWhileBuildingRequestType  = 6,
    LDASIInternalErrorWhileApplyingCredentialsType  = 7,
	LDASIFileManagementError = 8,
	LDASITooMuchRedirectionErrorType = 9,
	LDASIUnhandledExceptionError = 10,
	LDASICompressionError = 11
	
} LDASINetworkErrorType;


// The error domain that all errors generated by LDASIHTTPRequest use
extern NSString* const LDNetworkRequestErrorDomain;
...
extern unsigned long const LDASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^LDASIBasicBlock)(void);
typedef void (^LDASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^LDASISizeBlock)(long long size);
typedef void (^LDASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^LDASIDataBlock)(NSData *data);
#endif

@interface LDASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the LDASIHTTPRequestDelegate protocol
	id <LDASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but LDASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <LDASIHTTPRequestDelegate, LDASIProgressDelegate> queue;
	...
	// Automatically set to true in LDASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when LDASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = LDASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <LDASIProgressDelegate> uploadProgressDelegate;
	...
	id <LDASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force LDASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, LDASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, LDASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - LDASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	LDASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in LDASINetworkQueue.h
	...
	// Tells LDASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let LDASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// LDASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See LDASIAuthenticationState values above. 0 == default == No authentication needed yet
	LDASIAuthenticationState authenticationNeeded;
	
	// When YES, LDASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops LDASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be LDASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [LDASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <LDASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See LDASICacheDelegate.h for possible values
	LDASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See LDASICacheDelegate.h for possible values
	LDASICacheStoragePolicy cacheStoragePolicy;
	...
	LDASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using LDASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	LDASIHTTPRequest *PACFileRequest;
	...
	LDASIBasicBlock startedBlock;

	//block to execute when headers are received
	LDASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	LDASIBasicBlock completionBlock;

	//block to execute when request fails
	LDASIBasicBlock failureBlock;

	//block for when bytes are received
	LDASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	LDASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	LDASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	LDASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	LDASIDataBlock dataReceivedBlock;

	//block for handling authentication
	LDASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	LDASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    LDASIBasicBlock requestRedirectedBlock;
	#endif
}
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <LDASICacheDelegate>)cache;
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <LDASICacheDelegate>)cache andCachePolicy:(LDASICachePolicy)policy;

#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(LDASIBasicBlock)aStartedBlock;
- (void)setHeadersReceivedBlock:(LDASIHeadersBlock)aReceivedBlock;
- (void)setCompletionBlock:(LDASIBasicBlock)aCompletionBlock;
- (void)setFailedBlock:(LDASIBasicBlock)aFailedBlock;
- (void)setBytesReceivedBlock:(LDASIProgressBlock)aBytesReceivedBlock;
- (void)setBytesSentBlock:(LDASIProgressBlock)aBytesSentBlock;
- (void)setDownloadSizeIncrementedBlock:(LDASISizeBlock) aDownloadSizeIncrementedBlock;
- (void)setUploadSizeIncrementedBlock:(LDASISizeBlock) anUploadSizeIncrementedBlock;
- (void)setDataReceivedBlock:(LDASIDataBlock)aReceivedBlock;
- (void)setAuthenticationNeededBlock:(LDASIBasicBlock)anAuthenticationBlock;
- (void)setProxyAuthenticationNeededBlock:(LDASIBasicBlock)aProxyAuthenticationBlock;
- (void)setRequestRedirectedBlock:(LDASIBasicBlock)aRedirectBlock;
#endif
...
// Used by LDASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (LDASIHTTPRequest *)HEADRequest;
...
// Used internally to record bandwidth use, and by LDASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, LDASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
+ (void)setDefaultCache:(id <LDASICacheDelegate>)cache;
+ (id <LDASICacheDelegate>)defaultCache;
...
+ (NSThread *)threadForRequest:(LDASIHTTPRequest *)request;
...
@property (assign) BOOL useAllCookiesPersistence;
...
@property (assign) LDASIHTTPRequest *mainRequest;
...
@property (assign, readonly) LDASIAuthenticationState authenticationNeeded;
...
@property (assign) id <LDASICacheDelegate> downloadCache;
@property (assign) LDASICachePolicy cachePolicy;
@property (assign) LDASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) LDASIDataDecompressor *dataDecompressor;
